...
char * jsonControl( char * mac, char * cmd, int sid,
                    ...
                    int heat, int cool, int endpoint ) {
    jsonMessage[0] = '\0';
    catName( "ctrl" );
    catString( " : { " );
    catNameValueString( "mac", mac );
...
// Door lock
// ------------------------------------------------------------------------
char * jsonDoorLockControl( char * mac, char * cmd, int temp_password,
                    int hold_password) {
    jsonMessage[0] = '\0';
    catName( "doorlock" );
    catString( " : { " );
    ...
    if ( temp_password > 0 ) {
            catString( ", " );
            catNameValueInt( "tp", temp_password );
    }
    if ( hold_password >= 0 ) {
        catString( ", " );
        catNameValueInt( "hp", hold_password );
    }
